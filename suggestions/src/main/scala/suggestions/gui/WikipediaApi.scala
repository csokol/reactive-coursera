...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      val timed: Observable[T] = obs.take(totalSec seconds)
      val sub = PublishSubject[T]()
      timed.subscribe(new Observer[T] {
        override def onNext(value : T) { sub.onNext(value) }
        override def onError(error : Throwable) { sub.onError(error) }
        override def onCompleted() { sub.onCompleted() }
      })

      Observable.timer(totalSec seconds).subscribe(_ => sub.onCompleted())
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      obs.flatMap(t => {
        ...
        val request = requestMethod(t)
        request.subscribe(new Observer[S] {
          override def onError(t: Throwable) = subject.onNext(Failure(t))
          override def onNext(s: S) = subject.onNext(Success(s))
        })
        subject
      })
    }
